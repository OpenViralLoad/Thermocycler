...
long int CEIL_SEC = 30;
long int FLOOR_SEC = 60;
double MAINTAIN_DURATION = 0;
double MAINTAIN_DENAT = 1000 * 60 * 10; //Hold at hot temp for 10 minutes for denaturization
double MAINTAIN_CEILING =  1000 * CEIL_SEC; // Hold at hot temperature for 35 seconds
double MAINTAIN_FLOOR = 1000 * FLOOR_SEC; // Hold at cold temperature for 65 seconds
boolean DENAT;
...
void Maintain( int target )
{
  ...
  while ( stopTime - time > 0 )
  {
    ...
    nextPrint = PID( Thermistor( analogRead( 0 ), analogRead(1) ), target, first_run, time, nextPrint );
    first_run = false;
  }
  setLight( 0 ); // green
  Serial.println( "\n" );
...
void RampDown( int target )
{
  ...
  double startTime = millis();
  double ActualTemp = Thermistor( analogRead( pin_thermistorA0 ), analogRead( pin_thermistorA1 ) );
...
void wrapUp()
{
  ...
  {
    int newTemp = ( Thermistor( analogRead( 0 ), analogRead( 1 )  ) );
...
double Thermistor( int RawADC0, int RawADC1 )			// Return temperature reading from sensor
{
  ...
  double temp = SingleThermistor(RawADC0, A_Therm0, B_Therm0, C_Therm0) / 2 +
                SingleThermistor(RawADC1, A_Therm1, B_Therm1, C_Therm1) / 2;
  return temp;
