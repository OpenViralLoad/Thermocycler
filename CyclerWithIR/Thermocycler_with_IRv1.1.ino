...
void loop()
{
  long int MAINTAIN_CEILING =  35; // Hold at hot temperature for 35 seconds
  long int MAINTAIN_FLOOR = 65; // Hold at cold temperature for 65 seconds

  Denaturization( TEMP_CEILING );  // State 0: Denaturize samply by heating to TEMP_CEILING and Maintaining
  ...
  if ( current_cycle < MAX_CYCLES ) // More cycles to go
  {
    ...
    Maintain( TEMP_CEILING, MAINTAIN_CEILING );   // State 2: Keep sample heated at TEMP_CEILING for MAINTAIN_DURATION
    RampDown( TEMP_FLOOR );   // State 3: Cool down sample to TEMP_FLOOR
    Maintain( TEMP_FLOOR, MAINTAIN_FLOOR );   //State 2; Keep at Temp floor for duration
...
{
  long int MAINTAIN_DENAT = 60 * 10; //Hold at hot temp for 10 minutes for denaturization

  current_stage = "denat";
  if (DENAT)
  {
    double start = millis();
    Serial.println( F("Entering Denaturization Stage: 50C for 10 min, 95C for 5 min" ));
    UI.dispDenatStatus(0, 0);
    RampUp( 50 ); //Ramp up and maintain at 50C
    Maintain( 50, MAINTAIN_DENAT ); //Hold for denaturization stage at 50C
    RampUp( TEMP_CEILING ); //Ramp up to 95C
    Maintain( TEMP_CEILING, MAINTAIN_DENAT / 2 ); // Maintain for 5 min
    DENAT = 0;
...
*/
void Maintain( int target , long int duration)
{
  boolean first_run = true;
  Serial.print( "Maintaining temperature for: ");
  Serial.print(duration);
  Serial.println(" seconds");
  long int time = millis();
  long int stopTime = time + duration*1000;
  long int previous = 0;
  long int nextPrint = time + PRINT_DELAY;
  Serial.println(time);
